...
def get_flops_pytorch(model, input_sample, ost=sys.stdout,
    ...
    return int(flops_count), params_count, result
...
def get_model_flops_params(model, total_flops, total_params, 
                           flops_units: Optional[str] = 'GMac',
                           param_units: Optional[str] = 'M',
                           precision=3):
    ...
    # Attach methods to modules
    model.apply(add_extra)

    results = {}

    for name, m in model.named_modules():
        accumulated_params_num = m.accumulate_params()
        accumulated_flops_cost = m.accumulate_flops() / model.__batch_counter__

        results[name] = {
            'params': accumulated_params_num,
            'params_percent': accumulated_params_num / total_params,
            'params_str': params_to_string(accumulated_params_num,
                                           units=param_units, precision=precision),
            'flops': accumulated_flops_cost,
            'flops_percent': accumulated_flops_cost / total_flops,
            'flops_str': flops_to_string(accumulated_flops_cost,
                                         units=flops_units, precision=precision),
        }

    # Detach methods from modules
    model.apply(del_extra)
...
def attention_counter_hook(attention_module, input, output):
    ...
    if attention_module.spatial_norm is not None:
        flops += attention_module.spatial_norm.__flops__

    # GroupNorm
    if attention_module.group_norm is not None:
        flops += attention_module.group_norm.__flops__

    # NormCross
    if attention_module.norm_cross:
        flops += attention_module.norm_cross.__flops__

    flops += attention_module.to_q.__flops__
    flops += attention_module.to_k.__flops__
    flops += attention_module.to_v.__flops__
    ...
    flops += attention_module.to_out[0].__flops__
